...
var SECURITY_PRIVACY = function(){
	...
	const TWO_MINUTES = 2 * 60 * 1000; //2분
	const NEXT_ACTION_TIME_LOCAL_STORAGE_KEY = 'NEXT_ACTION_TIME';
	...
	var openChangePasswordNotice = function(month_notice){
		...
		$.ajax({
			...
			success : function(html){
				...
				$.cocoaDialog.open({type : "admin", custom_popup : $html, "close_block" : true});
			}
		});
	};
	...
	function showAutoLogoutAlert(auto_logout_time){
		...
		$.ajax({
			type : "POST",
			url : ("/admin/ajax/config/auto_logout_alert.cm"),
